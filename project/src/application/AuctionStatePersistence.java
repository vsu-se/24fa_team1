...
public class AuctionStatePersistence {
    ...
    public static void saveState(ObservableList<String> categories, ObservableList<Auction> auctions, double buyersPremium, double sellerCommission) {
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(SAVE_FILE_PATH))) {
            ...
            for (Auction auction : auctions) {
                writer.printf("%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%f|%b%n",
                		...
                		auction.getStartDate(), auction.getEndDate(), (auction.getBuyItNowPrice() == null ? "NULL" : String.format("%f", auction.getBuyItNowPrice())),
                            ...
                            case "ITEMS":
                                if (line.startsWith("BID")) {
									...
									Auction auction = new Auction(item, category, auctionData[5], auctionData[6], auctionData[7],
                                            ...
                                            (auctionData[10].equals("NULL") ? null : Double.parseDouble(auctionData[10])), Double.parseDouble(auctionData[11]), clock);
    ...
    public static void saveState(ObservableList<String> categories, ObservableList<Auction> auctions, double buyersPremium, double sellerCommission, String textFile) {
    	if(!canWrite) {
    		return;
    	}
    ...
    public static AuctionSystem loadState(String testFile) {
        ObservableList<String> categories = FXCollections.observableArrayList();;
        ObservableList<Auction> auctions = FXCollections.observableArrayList();;
        double buyersPremium = 0.0;
        double sellerCommission = 0.0;
