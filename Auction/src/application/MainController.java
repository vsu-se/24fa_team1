...
public class MainController {
    ...
    public void saveState() {
        PersistenceUtil.saveState(new ArrayList<>(categories), new ArrayList<>(items), buyersPremium, sellerCommission, clock.getTime());
    }

    public void loadState() {
        PersistenceUtil.SystemState state = PersistenceUtil.loadState(clock);
        categories.setAll(state.getCategories());
        items.setAll(state.getItems());
        buyersPremium = state.getBuyerPremium();
        sellerCommission = state.getSellerCommission();
        updateItemsDisplay();
        updateProfileItemsDisplay();
        updateConcludedAuctionsDisplay();
    }
        ...
        for (Item item : items) {
            if (selectedCategory == null || item.getCategory().equals(selectedCategory)) {
                if (item.isActive()) {
                    ...
                    itemBox.getChildren().addAll(bidAmountInput, placeBidButton, bidHistoryButton);
        ...
        for (Item item : items) {
        	...
        	if(item.hasBidder() && item.isActive()) { //Do not show inactive auctions with a bid.
                ...
                itemBoxProfile.getChildren().addAll(
                    ...
                    new Label("Active: " + (item.isActive() ? "Yes" : "No")),
                    new Label("Current Bid: $" + item.getCurrentBid()),
                    new Label("My Bid: $" + item.getCurrentBid()), 
                    bidHistoryButton
                );
                view.getMyProfileItemsBox().getChildren().add(itemBoxProfile);	
        	}
