...
import application.AuctionSystem;
...
class SystemTest17 {
	...
	void testLoadState() throws Exception {
        ...
        //Load state
        AuctionSystem system = AuctionStatePersistence.loadState("testFile.txt");
        
        //verify load
        assertTrue(system.getCategories().contains(category));
        assertTrue(system.getAuctions().get(0).getItem().getTitle().equals("Laptop"));
        assertEquals(1000, system.getAuctions().get(0).getBidHistory().get(0).getAmount());
        assertEquals(2000, system.getAuctions().get(0).getBidHistory().get(1).getAmount());
        assertEquals(system.getBuyersPremium(), 10.0);
        assertEquals(system.getSellerCommission(), 20.0);
